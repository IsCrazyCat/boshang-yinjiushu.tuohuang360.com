<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:49:"./application/admin/view2/goods\ajaxSpecList.html";i:1495107356;}*/ ?>
<table>
       <tbody>
			<?php if(is_array($specList) || $specList instanceof \think\Collection || $specList instanceof \think\Paginator): $i = 0; $__LIST__ = $specList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?>
              <tr>
                ...
                <td align="center" axis="col0">                  
                <div style="text-align: center; width: 50px;">
                  <input type="text" onKeyUp="this.value=this.value.replace(/[^\d]/g,'')" onpaste="this.value=this.value.replace(/[^\d]/g,'')" onChange="changeTableVal('spec','id','<?php echo $list['id']; ?>','order',this)" size="4" value="<?php echo $list['order']; ?>" />
                </div>                  
                </td>  
