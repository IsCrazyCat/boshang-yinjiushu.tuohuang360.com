<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:50:"./application/admin/view2/goods\ajaxGoodsList.html";i:1536744591;}*/ ?>
<table>
       <tbody>
            <?php if(is_array($goodsList) || $goodsList instanceof \think\Collection || $goodsList instanceof \think\Paginator): $i = 0; $__LIST__ = $goodsList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?>
			 ...
			 <td class="handle" >
                <div style="text-align:left;   min-width:50px !important; max-width:inherit !important;">
                  ...
                  <ul>
                    <li><a target="_blank" href="<?php echo U('Mobile/Goods/goodsInfo',array('id'=>$list['goods_id'])); ?>">预览商品</a></li>
                    <li><a href="<?php echo U('Admin/Goods/addEditGoods',array('id'=>$list['goods_id'])); ?>">编辑商品</a></li>
                    <li><a href="javascript:void(0);" onclick="del_goods('<?php echo $list[goods_id]; ?>',this)">删除商品</a></li>
                ...
                <td align="center" axis="col0">                  
                <div style="text-align: center; width: 50px;">
                  <input type="text" onKeyUp="this.value=this.value.replace(/[^\d]/g,'')" onpaste="this.value=this.value.replace(/[^\d]/g,'')" onChange="changeTableVal('goods','goods_id','<?php echo $list['goods_id']; ?>','sort',this)" size="4" value="<?php echo $list['sort']; ?>" />
                </div>                  
                </td>                     
