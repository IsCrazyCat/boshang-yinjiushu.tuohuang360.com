...
{    
    ...
    function getJSAPI($order){
    	if(stripos($order['order_sn'],'recharge') !== false){
    		...
    		$go_url = U('Mobile/User/order_detail_wxpay',array('id'=>$order['order_id']));
    		//$go_url = U('Mobile/Cart/cart4',array('order_id'=>$order['order_id']));
...
//        $input->SetOut_trade_no( $order['out_trade_no'] );
        $input->SetOut_trade_no( $order['order_sn'].time() );//lishibo by lishibo 20191115
