...
class Distribut extends MobileBase {
    ...
    public function index(){
		...
		if ( $money['achieve_money'] > 0  ) { 
			$data1['distribut_money'] = $money['achieve_money'];
			$row = M('users')->where(array('user_id'=>$this->user_id))->save($data1);
		}
		
        $money['prepare_money'] = Db::name('rebate_log')->where("user_id = ".$this->user_id." and status > 0 and status < 3")->sum('money');  //冻结佣金
		if ( $money['prepare_money'] > 0 ) { 
			$data2['frozen_money'] = $money['prepare_money'];
...
//		M("users")->where("user_id", $this->user_id)->save($vv);	
        ...
        $this->assign('lower_count',$lower_count); // 推荐人数        
   ...
   /**
    ...
    public function getTargetLowerList_20181126(){
      ...
      //printf("目标用户： %s ", $targetName);
      $user = $this->user;
      $where = "( first_leader = {$targetID} ) ";
      ...
      foreach($lists as $key => $value){
        $lists[$key]['yjnum'] = Db::name('users')->where(" first_leader = '".  $lists[$key]['user_id'] ."' ")->count();			
      }	
      ...
      $this->assign('lists',$lists); // 代理
      $this->assign('mycount', $mycount);
      $this->assign('targetObj', $targetObj);
    ...
    public function must_see(){
        $article = D('article')->field('article_id,title,content')->where( "article_id = 1414 or article_id = 1413")->order("article_id desc")->cache(true)->select();
...
//        $count = Db::name('users')->where("is_distribut = 1")->count(); //统计符合条件的总数
    ...
    /**
     * 分成记录 分页，条件传递
     * @author lishibo  2018-12-11
     * @return mixed
     * @throws \think\db\exception\DataNotFoundException
     * @throws \think\db\exception\ModelNotFoundException
     * @throws \think\exception\DbException
     */
    public function rebate_log_20181211(){
        $user =$this->user;
        if($user['is_distribut'] != 1)
            $this->error('您还不是分销商');
        $status = I('status',''); //日志状态
	...
	public function rebate_log2(){
        ...
        $count = Db::name('rebate_log')->where($where)->count(); //统计符合条件的数量
        $Page = new Page($count,C('PAGESIZE'));
        $myfirstRow = $Page->firstRow + $Page->listRows;
        $lists = Db::name('rebate_log')
            ->where($where)->order("$sort  $sort_asc")
            ->limit($myfirstRow.','.$Page->listRows)
            ->cache(true)
            ->select(); //查询日志
        $this->assign('lists',$lists);
        $page1 = new Page($count);
