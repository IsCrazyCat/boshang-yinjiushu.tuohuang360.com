...
//		DB::name('Cart')->where("member_goods_price", 81.92)->save(['member_goods_price' => 102.40]);
//		DB::name('Cart')->where("member_goods_price", 163.84)->save(['member_goods_price' => 204.8]);
		
        M("Cart")->where("member_goods_price >= 80 and member_goods_price <= 85  ")->delete(); 
		M("Cart")->where("member_goods_price >= 158 and member_goods_price <= 165 ")->delete(); 
	...
	// 订单满额优惠活动		                
        ...
        $car_price = array(
            ...
            'payables'     => $result['result']['order_amount'], // 应付积分
...
//                }
    ...
    public function cart4(){
        ...
        if($order['pay_status'] == 1){
			M('rebate_log')->where("order_id = ".$order_id." and `status` = 1 ")->save(array("status"=>3));
...
//        M("Cart")->where("user_id = 0 and  add_time < $time")->delete(); 
        ...
        if ($post_goods_num) {
            ...
            foreach ($post_goods_num as $key => $val) {
				...
				$mylevel = (int)M('users')->where($where)->getField("level");
//						$mydiscount = (int)M('user_level')->where('level_id = '.$mylevel)->getField("discount");
//						if ( (int)$mydiscount > 0 ) {
//							$my_price = $goods['shop_price'] / 100 * $mydiscount;	
//						} else {
//							$mydiscount = 100 ;  	
//						}
					
					if ( $mylevel == 1  ) {
						if ( (int)$data['goods_num'] < 10  ) {
							$mydiscount = M('users')->where($where)->getField("discount");
							if ( $mydiscount ) {
								$data['member_goods_price'] = $data['goods_price'] = $goods['shop_price'] * $mydiscount; 	
							} else {
								$data['member_goods_price'] = $data['goods_price'] = $goods['shop_price'];		
							}
						} elseif ( ( (int)$data['goods_num'] >= 10  ) and ( (int)$data['goods_num'] < 30  ) ) {
							$data['member_goods_price'] = $data['goods_price'] = $goods['shop_price'] * 0.78125;	
						} elseif (  (int)$data['goods_num'] >= 30  ) {
							$data['member_goods_price'] = $data['goods_price'] = $goods['shop_price'] * 0.53125;	
						}   /*
						    elseif ( ( (int)$data['goods_num'] >= 30 ) and ( (int)$data['goods_num'] < 39  ) ) {
							$data['member_goods_price'] = $data['goods_price'] = $goods['shop_price'] * 0.53125;	
						}
						    elseif ( (int)$data['goods_num'] >= 39 ) {
							$data['member_goods_price'] = $data['goods_price'] = $goods['shop_price'] * 0.40625;	
						}*/
					} elseif ( $mylevel == 2  ) {
						if ( (int)$data['goods_num'] < 20  ) {
							$mydiscount = M('users')->where($where)->getField("discount");
							if ( $mydiscount ) {
								$data['member_goods_price'] = $data['goods_price'] = $goods['shop_price'] * $mydiscount; 	
							} else {
