...
class User extends MobileBase
{
    ...
    */
    ...
    public function index()
    {
		...
		$prepare_money = Db::name('rebate_log')->where("user_id = ".$this->user_id." and status > 0 and status < 3")->sum('money');  //冻结佣金
        $this->assign('prepare_money',$prepare_money);
		if ( $prepare_money > 0 ) { 
			$data1['frozen_money'] = $prepare_money;
			$row = M('users')->where(array('user_id'=>$this->user_id))->save($data1);
		}
		
		$distribut_money = Db::name('rebate_log')->where("user_id = ".$this->user_id." and status = 3")->sum('money');  //冻结佣金
        $this->assign('distribut_money',$distribut_money);
		if ( $distribut_money > 0  ) { 
			$data2['distribut_money'] = $distribut_money;
			$row = M('users')->where(array('user_id'=>$this->user_id))->save($data2);
		}
    ...
    public function checkcoupon()
    {
        ...
        // 找出这个用户的优惠券 没过期的  并且 订单积分达到 condition 优惠券指定标准的
...
//            $cartLogic->login_cart_handle($this->session_id, $res['result']['user_id']);  //用户登录后 需要对购物车 一些操作
    ...
    public function reg()
    {
        ...
        if (IS_POST) {
            ...
            $chushengriqi = I('post.chushengriqi', '');
			//$chushengriqi = strtotime($chushengriqi);
			
			$invite = 0;
			$invite = I('post.invite', 0);//推荐人手机号
            ...
            $data = $logic->my_reg($username,$mobile,$password, $password2,$chushengriqi, 0 , trim($invite) );
            if ($data['status'] != 1)
...
//          $cartLogic->login_cart_handle($this->session_id, $data['result']['user_id']);  //用户登录后 需要对购物车 一些操作
     ...
     * @autho lishibo
     */
    public function order_detail_wxpay()
    {
        $id = I('get.id/d');
        $map['order_id'] = $id;
      ...
      /*  if($order_info['pay_status'] == 1){
            ...
            $update = Db::name('rebate_log')->where('order_id',$map['order_id'])->update($update_data);
          ...
          //   $order_detail_url = U("Mobile/User/order_detail",array('id'=>$map['order_id']));
           //  header("Location: $order_detail_url");

           // header('Location:' . U('/Mobile/User/order_detail', array('address_id' =>$map['order_id'])));
            header('Location:' . U('Mobile/User/order_list'));
            exit;
        }*/
...
//		}
    ...
    public function return_goods()
    {
        ...
        if(IS_POST)
        {
            ...
            $res = $model->addReturnGoods($order_id,$order,$describe);  //退款退货
			
			$mytype = I('type',0);
			if ( (int)$mytype< 2 ) {
				M("rebate_log")->where("order_id = ".$order_id." and status < 3 ")->delete(); 
			}
...
//            $this->verifyHandle('withdrawals');
            ...
            if ($data['money'] > $this->user['user_money']) {
                $this->error("你最多可提现{$this->user['user_money']}可用余额.");
                exit;
            }
			...
			$data['sxf'] = round($data['money'] * 0.006,2); ;
			$data['ydkje'] = $data['money'] - $data['sxf'] ;
			$data['ydkje'] = round($data['ydkje'],2);
            if (M('withdrawals')->add($data)) {
                //$this->success("已提交申请");
				$this->success("已提交申请,24小时内打款", U('User/Index'));
                exit;
            } else {
        ...
        $this->assign('user_money', $this->user['user_money']);    //可用余额
        return $this->fetch();
    }
