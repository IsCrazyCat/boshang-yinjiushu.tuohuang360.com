...
class DistributLogic //extends Model
{
     ...
     {       
         ...
         if($user['first_leader'] > 0 && $first_money > 0.01)
         {
            ...
            {
                $wx_content = "你的一度人脉,刚刚下了一笔订单:{$order['order_sn']} 如果交易成功你将获得 ￥".$first_money."奖励 !";
         ...
         {         
            ...
            if($tmp_user['oauth']== 'weixin')
            {
                $wx_content = "你的二级人脉,刚刚下了一笔订单:{$order['order_sn']} 如果交易成功你将获得 ￥".$second_money."奖励 !";
                $jssdk->push_msg($tmp_user['openid'],$wx_content);
            }              
         }
          // 三级 分销商赚 的钱.
         ...
         {                  
            ...
            if($tmp_user['oauth']== 'weixin')
            {
                $wx_content = "你的三级人脉,刚刚下了一笔订单:{$order['order_sn']} 如果交易成功你将获得 ￥".$thirdmoney."奖励 !";
                $jssdk->push_msg($tmp_user['openid'],$wx_content);
            }              
