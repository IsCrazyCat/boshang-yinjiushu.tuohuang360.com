...
class OrderLogic
{
	public function addReturnGoods($rec_id,$order,$describe)
	{
		$data = I('post.');
		$confirm_time_config = tpCache('shopping.auto_service_date');//后台设置多少天内可退款退货
    ...
    public function recordRefundOrder($user_id, $order_id, $user_note, $consignee, $mobile)
    {
    	...
    	if($order_return_num > 0){
    		return ['status' => -1, 'msg' => '该订单中有商品正在退款退货'];
    	}
    	$order_status = 3;//已取消
    	$order_info = [
    	'user_note' => $user_note,
