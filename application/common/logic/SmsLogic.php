...
class SmsLogic 
{
    ...
    public function sendSms($scene, $sender, $params, $unique_id=0)
    {
        ...
        $sms_sign = $params['sms_sign']; 
        $duoshaoyuan = $params['duoshaoyuan']; 
		$duoshaoyongjin = $params['duoshaoyongjin']; 
...
//            6 => "{\"user_name\":\"$user_name\",\"code\":\"$code\"}",                                                            //6. 修改手机号码
            ...
            1 => "{\"code\":\"$code\"}",            //1. 用户注册
            2 => "{\"code\":\"$code\"}",           //2. 用户找回密码
//            3 => "{\"user_name\":\"$user_name\",\"duoshaoyuan\":\"$duoshaoyuan\"}",   //3. 客户下单
//            4 => "{\"order_sn\":\"$order_sn\"}",     //4. 客户支付
//            5 => "{\"user_name\":\"$user_name\",\"order_sn\":\"$order_sn\",\"consignee\":\"$consignee\"}",  //5.商家发货
            3 => "{\"consignee\":\"$consignee\",\"phone\":\"$mobile\"}",                                                       //3. 客户下单
            4 => "{\"order_sn\":\"$order_sn\"}",                                                                                               //4. 客户支付
            5 => "{\"user_name\":\"$user_name\",\"order_sn\":\"$order_sn\",\"consignee\":\"$consignee\"}",  //5.商家发货
            6 => "{\"code\":\"$code\"}",            //6. 修改手机号码
            10 => "{\"user_name\":\"$user_name\",\"sms_sign\":\"$sms_sign\",\"duoshaoyongjin\":\"$duoshaoyongjin\"}",            //10. 消费返佣金
            11 => "{\"user_name\":\"$user_name\"}",            //11. 会员生日祝福
            12 => "{\"user_name\":\"$user_name\",\"duoshaoyuan\":\"$duoshaoyuan\",\"yue\":\"$yue\"}",            //12. 股东消费
