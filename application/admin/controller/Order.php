...
class Order extends Base {
    ...
    */
    public function index0(){
        ...
        I('pay_name') != '' ? $condition['pay_name'] = I('pay_name') : false;
        ...
        /**************分页附带搜索参数 lishibo 20190523*********************/
        $order_status=I('order_status') != '' ? I('order_status'):NULL;
        $shipping_status = I('shipping_status') != '' ? I('shipping_status'):NULL;
        $pay_name = I('pay_name') != '' ? I('pay_name'):NULL;
        $pay_status = I('pay_status') != '' ? I('pay_status'):NULL;
        $this->assign('keytype',$keyType);
        $this->assign('keywords',$keywords);
        $this->assign('order_status_',$order_status);
        $this->assign('shipping_status',$shipping_status);
        $this->assign('pay_name',$pay_name);
        $this->assign('pay_status',$pay_status);
        $this->assign('begin',I('add_time_begin'));
        $this->assign('end',I('add_time_end'));
    ...
    */
    ...
    public function detail($order_id){
		...
		if ($order['user_id']>0) {
			$myuser = M('users')->where("user_id", $order['user_id'])->find();
			$order['username'] = $myuser['username'];
			$order['nickname'] = $myuser['nickname'];
			if ( (int)$myuser['first_leader'] == 0 ) {
				$order['tjr_user_id']  = 0;
				$order['tjr_username'] = '官网';
				$order['tjr_nickname'] = '';	
			}
			if ( (int)$myuser['first_leader'] > 0 ) {
				$tjruser = M('users')->where("user_id", $myuser['first_leader'])->find();
				$order['tjr_user_id']  = $tjruser['user_id'];
				$order['tjr_username'] = $tjruser['username'];
				$order['tjr_nickname'] = $tjruser['nickname'];
			}
    ...
    public function pay_cancel($order_id){
    	if(I('remark')){
    		$data = I('post.');
    		$note = array('退款到可用余额','已通过其他方式退款','不处理，误操作项');
    		if($data['refundType'] == 0 && $data['amount']>0){
    			accountLog($data['user_id'], $data['amount'], 0,  '退款到可用余额');
    		}
    		$orderLogic = new OrderLogic();
    	...
    	if($order['pay_code'] == 'weixin' || $order['pay_code'] == 'alipay' || $order['pay_code'] == 'alipayMobile'){
    		...
    		if($order['pay_code'] == 'weixin'){
    			...
    			$data = array(
                   ...
                   'refund_fee'=>$return_money*100
               );

             // echo($data["out_trade_no"].'-----：'.$data["total_fee"].'-------退款：'.$data["refund_fee"]);
              $result = $Payment_obj->payment_refund($data);
              ...
              if($result['return_code'] == 'SUCCESS'){
    				...
    				//退款订单状态修改 lishibo
                   $orderLogic = new OrderLogic();
                   $res = $orderLogic->alterReturnGoodsStatus($return_id,$return_goods['order_id']);
                   $orderLogic->closeOrderByReturn($return_goods['order_id']);
                   //如果有分成订单，取消分成 lishibo
                  M('rebate_log')->where("order_id" ,$return_goods['order_id'])->save(array('status'=>4));
    ...
    public function add_return_goods()
   {
    ...
    public function export_order()
    {
      ...
      $orderList = Db::name('order')->field("*,FROM_UNIXTIME(add_time, '%Y/%m/%d %H:%i:%s')  as create_time ")->where($where)->order('order_id')->select();
        ...
        $strTable .= '<td style="text-align:center;font-size:12px;width:120px;">会员编号</td>'; //lishibo
