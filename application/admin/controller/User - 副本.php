...
//		}
    ...
    public function detail(){
        ...
        if(IS_POST){
			...
			/*会员分佣开始 进单佣金 */
/*			if( ($_POST['is_distribut']==1) && ($user['sfffyj']==0) ){
				 ...
				 if($user_level){
					 if ( (int)$user_level['amount']>0 ) {
						$commission = $user_level['amount'];
						...
						$thirdmoney = $commission * ($third_rate / 100); // 三级赚到的钱
						 ...
						 if($user['first_leader'] > 0 && $first_money > 0.01)
						 {
							...
							accountLog($user['first_leader'], $first_money, 0,"您的一级代理 ".$user['nickname']."开通分销商，产生的分佣",$first_money);   
						 }
						  // 二级 分销商赚 的钱.
						 ...
						 {         
							...
							M('rebate_log')->add($data);
							accountLog($user['second_leader'], $second_money, 0,"您的二级代理 ".$user['nickname']."开通分销商，产生的分佣",$second_money);   
						 ...
						 {                  
							...
							M('rebate_log')->add($data);      
							accountLog($user['third_leader'], $thirdmoney, 0,"您的三级代理 ".$user['nickname']."开通分销商，产生的分佣",$thirdmoney);   

						 }						 
						 
						M('users')->where('user_id = '.$user['user_id'])->setField('sfffyj','1');
						 
						//exit($this->error( 'AAAAAAAAA'.$first_money.'BBBBBBB'.$second_money.'CCCCCCC'.$thirdmoney ));
					 } 
				 }
												...
												if($user_3){
													...
													//if($res && $res['status'] ==1 && !empty($sender)){
														//if ($sender != '15314170988') {  
															...
															$params = array('user_name'=>$user['nickname'] ,'sms_sign'=>'饮久舒新时代','duoshaoyongjin'=>$zongsssjyj  );
