...
class Distribut extends Base {
    ...
    public  function ajax_lower()
    {
		...
		foreach($list as $k=>$val){ 
			$where1['first_leader'] = array('eq',$val['user_id']);
			$count1 = 0;
    ...
    public  function editRebate(){        
        ...
        if (IS_POST) {
            ...
            if ($data['status'] == 3 && $rebate_log['status'] != 3) {
                accountLog($data['user_id'], $rebate_log['money'], 0, "订单:{$rebate_log['order_sn']}分佣", 0-$rebate_log['money'],$rebate_log['order_id'],$rebate_log['order_sn']);
