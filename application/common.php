...
*/
    $total_amount = M('order')->master()->where("user_id=:user_id AND pay_status=1 and order_status not in (3,5)")->bind(['user_id'=>$user_id])->sum('order_amount+user_money');
	$user_level = M('users')->where(['user_id' => $user_id])->getField('level');
    $mycount = M('order')->where( "user_id =".$user_id." AND pay_status=1 and order_status not in (3,5) and  `order_amount` + `user_money`  >= 2000 ")->count(); 
	//$count = M('recharge')->where(['order_sn'=>$order_sn,'pay_status'=>0])->count(); 
	$mycount3 = M('order')->where( "user_id =".$user_id." AND pay_status=1 and order_status not in (3,5) and  `order_amount` + `user_money`  >= 4056 ")->count(); 
	$mycount4 = M('order')->where( "user_id =".$user_id." AND pay_status=1 and order_status not in (3,5) and  `order_amount` + `user_money`  >= 50024 ")->count(); 
	$updata['total_amount'] = $total_amount;//更新累计修复额度
	if  ( $user_level == 1 ) {
		if( $mycount >= 1){
            $updata['level'] = 2;
			$level_info = M('user_level')->where("level_id", 2)->find();
            $updata['discount'] = $level_info['discount']/100;
        }
		if( $mycount3 >= 1){
            $updata['level'] = 3;
			$level_info = M('user_level')->where("level_id", 3)->find();
            $updata['discount'] = $level_info['discount']/100;
        }
		if ( $mycount4 >= 1 ) {
            $updata['level'] = 4;
			$level_info = M('user_level')->where("level_id", 4)->find();
            $updata['discount'] = $level_info['discount']/100;
        }
	}
	if  ( $user_level == 2 ) {
	...
	if ( (int)$user_level == 2 && $total_amount >= 45968 ) {
		$updata['level'] = 4;
		$level_info = M('user_level')->where("level_id", 4)->find();
		$updata['discount'] = $level_info['discount']/100;
	}
	
	if ( $total_amount >= 50024 ) {
		$updata['level'] = 4;
		$level_info = M('user_level')->where("level_id", 4)->find();
		$updata['discount'] = $level_info['discount']/100;
    }
 ...
 * @param   int     $pay_points     用户积分变动
 * @param   string  $desc    变动说明
 * @param   float   distribut_money 分佣积分
...
function update_pay_status($order_sn,$ext=array())
{
    if(stripos($order_sn,'recharge') !== false){
        ...
        // 给他升级, 根据order表查看消费记录 给他会员等级升级 修改他的折扣 和 总积分
        //update_user_level($order['user_id']);
        // 记录订单操作日志
...
function get_goods_promotion2($goods_id,$user_id=0){
    ...
    if($goods['prom_type'] == 3){//优惠促销
        $parse_type = array('0'=>'直接打折','1'=>'减价优惠','2'=>'固定积分出售','3'=>'买就赠优惠券','4'=>'买M件送N件');
        ...
        if(!empty($prominfo)){
            if($prominfo['type'] == 0){
                ...
                $prom['price'] = $prominfo['expression'];//固定积分优惠
            }
        }
    }
...
{
//    $parse_type = array('0'=>'满额打折','1'=>'满额优惠积分','2'=>'满额送倍数积分','3'=>'满额送优惠券','4'=>'满额免运费');
    $now = time();
    ...
    if ($prom) {
        if ($prom['type'] == 0) {
            ...
            $res['order_amount'] = $order_amount - $prom['expression'];//满额优惠积分
...
function calculate_price($user_id = 0, $order_goods, $shipping_code = '', $shipping_price = 0, $province = 0, $city = 0, $district = 0, $pay_points = 0, $user_money = 0, $coupon_id = 0, $couponCode = '')
{
    ...
    $use_percent_point = tpCache('shopping.point_use_percent') / 100;     //最大使用限制: 最大使用积分比例, 例如: 为50时, 未50% , 那么积分支付抵扣积分不能超过应付积分的50%
    ...
    $order_amount = $goods_price + $shipping_price - $coupon_price; // 应付积分 = 商品价格 + 物流费 - 优惠券

    $user_money = ($user_money > $order_amount) ? $order_amount : $user_money;  // 余额支付原理等同于积分
    $order_amount = $order_amount - $user_money; //  余额支付抵应付积分

    /*判断能否使用积分
     1..积分低于point_min_limit时,不可使用
     2.在不使用积分的情况下, 计算商品应付积分
     3.原则上, 积分支付不能超过商品应付积分的50%, 该值可在平台设置
     @{ */
    ...
    $order_amount = $order_amount - $pay_points; //  积分抵消应付积分

    $total_amount = $goods_price + $shipping_price;
    //订单总价  应付积分  物流费  商品总价 节约积分 共多少件商品 积分  余额  优惠券
    $result = array(
        'total_amount' => $total_amount, // 商品总价
        'order_amount' => $order_amount, // 应付积分
        'shipping_price' => $shipping_price, // 物流费
        'goods_price' => $goods_price, // 商品总价
        'cut_fee' => $cut_fee, // 共节约多少钱
        'anum' => $anum, // 商品总共数量
        'integral_money' => $pay_points,  // 积分抵消积分
        'user_money' => $user_money, // 使用余额
        'coupon_price' => $coupon_price,// 优惠券抵消积分
