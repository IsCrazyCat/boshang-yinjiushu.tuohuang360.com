...
class CartLogic extends Model
{
    ...
    private function addNormalCart($goods_num,$goods_spec_key){
        ...
        if ($userCartGoods) {
			...
			$mydiscount = M('users')->where('user_id',$this->user_id)->getField("discount");
			if ( $mydiscount ) {
				$member_goods_price = $pricea = $price * $mydiscount; 	
			} else {
				$member_goods_price = $pricea = $price; 		
			}
			
            $cartResult = $CartModel->save(['goods_num' => $userWantGoodsNum,'goods_price'=>$pricea,'member_goods_price'=>$member_goods_price], ['id' => $userCartGoods['id']]);
        }else{
            //如果有阶梯价格
            if (!empty($goods['price_ladder'])) {
                $price_ladder = unserialize($goods['price_ladder']);
                ...
                $price = isset($specPrice) ? $specPrice : $this->goods['shop_price'];
			        ...
			        $mylevel = (int)M('users')->where('user_id',$this->user_id)->getField("level");
					if ( $mylevel == 1  ) {
						if ( (int)$goods_num < 10  ) {
							$member_goods_price = $pricea = $price * 1;		
						} elseif ( ( (int)$goods_num >= 10  ) and ( (int)$goods_num < 30 ) ) {
							$member_goods_price = $pricea = $price  * 0.78125;	
						} elseif (  (int)$goods_num >= 30    )  {
							$member_goods_price = $pricea = $price  * 0.53125;	
						} /*
						elseif ( ( (int)$goods_num >= 30  ) and ( (int)$goods_num < 39 )   )  {
							$member_goods_price = $pricea = $price  * 0.53125;	
						}
						elseif ( (int)$goods_num >= 39  ) {
							$member_goods_price = $pricea = $price  * 0.40625;
						}*/
					} elseif ( $mylevel == 2  ) {
						if ( (int)$goods_num < 20  ) {
							$member_goods_price = $pricea = $price * 0.78125;		
						} elseif (  (int)$goods_num >= 20   ) {
							$member_goods_price = $pricea = $price * 0.40625;		
						}
					} 
...
//		DB::name('Cart')->where("member_goods_price", 163.84)->save(['member_goods_price' => 204.8]);
        ...
        $total_goods_num = $total_price = $cut_fee = 0;//初始化数据。商品总共数量/商品总额/节约积分
