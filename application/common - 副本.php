...
function update_user_level($user_id){
	...
	$myuser = M('users')->where("user_id", $user_id)->find();//李洪顺
    if($level_info){
        ...
        if(isset($level) && $level>$user['level']){
			...
			//李洪顺 2018.11.3 
			//20位业绩代表(2)可升级为公司经销商（3）
			$dqbsf = 0;
			$dqbsf = intval($total_amount/39.9); //当前 多少份
			//查询公司会员数
			$where1['first_leader'] = array('eq',$user_id);
			$where1['level'] = array('lt',3);
			$count1 = 0; 
			$count1 = M('users')->where($where1)->count(); 
			//$count1 = M('users')->where(" first_leader=:user_id AND level>=1 ")->count();
			if ( $count1 + $dqbsf >= 21 ) { //21 
				$updata['level'] = 3;
				$updata['discount'] = '1.00';
				$updata['sfffyj3'] = 1;
			}		
		...
		$where2['first_leader'] = array('eq',$user_id);//成功培养一名公司经销商后
		$where2['level'] = array('eq',3);
		$count2 = 0;
		$count2 = M('users')->where($where2)->count();	
		...
		$where3['first_leader'] = array('eq',$user_id);
		$where3['level'] = array('eq',3);
		$count3 = 0;
		$count3 = M('users')->where($where3)->count();			
		if ( (int)$count3 >= 20 ) { 
			if ($myuser['level'] != 4) {
				$updata['level'] = 4;
				$updata['discount'] = '1.00';
...
function my_accountLog($user_id, $user_money = 0,$pay_points = 0, $desc = '',$distribut_money = 0,$order_id = 0 ,$order_sn = '',$address_id,$total_amount = 0,$jxmc=''){
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id'       => $user_id,
        'user_money'    => $user_money,
...
//        " pay_points = pay_points + $pay_points, distribut_money = distribut_money + $distribut_money WHERE user_id = $user_id";
    $update_data = array(
        'user_money'        => ['exp','user_money+'.$distribut_money],
        'pay_points'        => ['exp','pay_points+'.$pay_points],
        'distribut_money'   => ['exp','distribut_money+'.$distribut_money],
        'total_amount'   => ['exp','total_amount+'.$total_amount],
		
    );
  if(($user_money+$pay_points+$distribut_money+$total_amount) == 0)
      return false;
    M('account_log')->add($account_log);
    $update = Db::name('users')->where('user_id',$user_id)->update($update_data);
    if($update){
